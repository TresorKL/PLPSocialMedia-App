    ...
    <div class="container align-items-center ">
        ...
        @foreach (Session::get('posts') as $post)
            @if ($post->image != null)
                            ...
                            <div class="card-body">
                                <div class="row">
                                    <form action="/delete/{{ $post->id }}" method="GET">
                                        @csrf
                                        <button type="submit" class="btn btn-danger m-3" v
                                            data-toggle="modal">Delete</button>
                                    </form>
                                    <button type="button" class="btn btn-dark m-3" data-toggle="modal"
                                        data-target="#EditCaptionModal{{ $post->id }}">Edit</button>
                ...
                <!-- EDIT CAPTION Modal -->
                <div class="modal fade" id="EditCaptionModal{{ $post->id }}" tabindex="-1" role="dialog"
                    aria-labelledby="newPostModalLabel" aria-hidden="true">
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="newPostModalLabel">EDIT POST CAPTION</h5>
                                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                    <span aria-hidden="true">&times;</span>
                                </button>
                            </div>
                            <div class="modal-body">
                                <form action="/edit/{{ $post->id }}" method="POST">
                                    ...
                                    <div class="form-group">
                                        <label for="caption">Caption</label>
                                        {{-- <input type="text" class="form-control" id="caption" placeholder="Enter caption"> --}}
                                        <textarea name="caption" type="text" class="form-control" id="caption" placeholder="Say something" required>{{ $post->caption }}</textarea>
                                    </div>
